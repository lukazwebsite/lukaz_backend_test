<script setup lang="ts">
    ...
    const pageNumber = ref(props.staffs?.current_page);

    const breadcrumbs = ref([
        { title: 'Dashboard', href: '/' },
        { title: 'Staffs', href: '/staff' }
    ]);
...
const submit = () => {
        ...
        router.post('/staff/paginate/filters', formData, {
            headers: {
    ...
    const goToPage = () => {
        ...
        router.get('/staff?page=' + pageNumber.value, {}, {
            headers: {
                Accept: 'application/json',
                'X-Requested-With': 'XMLHttpRequest',
            },
        ...
        <div class="w-full mx-auto p-2 flex flex-row place-content-center">
            <div className="w-5/6 mt-4 shadow-lg bg-white rounded-md pb-1">
                <div class="flex justify-between bg-gray-300 px-2 py-1 items-center rounded-t-md">
                    <div class="text-lg font-semibold">Staff List</div>
                    <div class="flex">
                        <div :class="{'rounded-md': !checkPermission }" class="bg-sky-600 items-center p-2 flex px-4 rounded-l-md cursor-pointer" @click="visibleRight = true">
                            <Icon icon="mdi:filter-outline" width="1.5rem" height="1.5rem" class="text-white"  />
                            <div class="px-2 text-white text-lg ">Filter</div>

                        </div>

                        <Link v-if="add?.action_id || checkPermission" href="/staff/create" class="bg-green-600 p-2 rounded-r-md text-lg px-3 flex text-black text-white">
                            <Icon icon="fluent:add-12-filled" class="mr-2" width="1.5rem" height="1.5erm"/> Add Staff </Link>
                    </div>
                </div>
                ...
                <div class="px-3 h-[calc(100vh-13rem)] overflow-auto pb-3">
                    <div class="w-full border-t mt-4">
                        <table class="w-full">
                            ...
                            <tbody>
                                <tr v-for="(staff, index) in Object.values(staffs?.data)" class="odd:bg-white even:bg-gray-200 text-gray-800">
                                    <td class="text-center">{{ ((staffs?.current_page - 1) * staffs?.per_page) + (index + 1) }}</td>
                                    ...
                                    <td class="p-2">{{ dateFunction(staff?.created_at) }}</td>
                                    <td class="p-2">{{ (staff?.status) ? 'Active' : 'Inactive' }}</td>
                                    <td class="flex gap-1 place-content-center">
                                        <Link v-if="edit?.action_id || checkPermission" :href="`/staff/${staff?.id}/edit`">
                ...
                <!-- main content goes here -->
                <Drawer v-model:visible="visibleRight" header="Filter Option" position="right">
                    <form @submit.prevent="submit">
                        <div class="grid grid-col-1 mt-4">
                            ...
                            <div class="card w-full justify-center">
                                <label for="dd-city" class="text-md w-full content-center font-semibold">Role</label>
                                <select v-model="roleId" class="w-full text-md border py-1 px-2 outline-none focus:border-green-200 rounded-md">
                                    <option v-for="role in roles" :value="role?.id">{{ role?.name }}</option>
                                </select>
                            </div>

                             <div class="card w-full justify-center">
                                <label for="dd-city" class="text-md w-full content-center font-semibold">Branch</label>
                                <select v-model="branchId" class="w-full text-md border py-1 px-2 outline-none focus:border-green-200 rounded-md">
                                    <option v-for="item in branches" :value="item?.id">{{ item?.name }}</option>
                                </select>
                            </div>
