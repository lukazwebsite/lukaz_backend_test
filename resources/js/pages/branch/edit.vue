<script setup lang="ts">
    ...
    const nameBn = ref(props.branch?.name_bn);
    const contact = ref(props.branch?.contact);
    const managerPhone = ref(props.branch?.manager_phone);
    const address = ref(props.branch?.address);
    const addressBn = ref(props.branch?.address_bn);
    const mapLink = ref(props.branch?.map_link);
    const mapEmbed = ref(props.branch?.map_embed);
    const status = ref(props.branch?.status);
    const image = ref(props.branch?.image);
    ...
    const submit = () => {
        ...
        router.post('/branches/'+id.value, formData, {
            headers: {
        ...
        <div class="w-full mx-auto p-2 flex flex-row place-content-center">
            <div className="w-5/6 mt-4 shadow-lg bg-white rounded-md pb-1">
                ...
                <div class="flex justify-between bg-gray-300 px-2 items-center rounded-t-md p-1 ">
                    <div class="text-lg font-semibold">Edit <span class="italic"> {{name}}</span></div>
                    <div class="flex">
                ...
                <!-- main content goes here -->
                    ...
                    <form @submit.prevent="submit" class="px-3 h-[calc(100vh-10rem)] justify-items-center mt-2">
                        ...
                        <div class="w-1/2 shadow-sm rounded-md border-t p-2 px-4">
                            ...
                            <div class="w-full mb-1">
                                <label for="dd-city" :class="{'text-green-500': image != null }" class="`text-md font-semibold w-full content-center">Outlet Image:</label>
                            ...
                            <div class="w-full mb-1 grid grid-flow-col justify-items-end mt-2">
                                ...
                                <button class="justify-items-end cursor-pointer bg-orange-500 hover:bg-orange-600 text-md border py-1 px-2 rounded-md font-semibold text-white">Update Branch</button>
                            </div>
