...
onMounted(() => {
    ...
    const pageNumber = ref(props.brands?.current_page);
...
const submit = () => {
        ...
        router.post('/brands/paginate/filters', formData, {
            headers: {
    ...
    const goToPage = () => {
        ...
        router.get('/brands?page=' + pageNumber.value, {}, {
            headers: {
                Accept: 'application/json',
                'X-Requested-With': 'XMLHttpRequest',
            },
        ...
        <div class="w-full mx-auto p-2 flex flex-row place-content-center">
            <div className="w-5/6 mt-4 shadow-lg bg-white rounded-md pb-1">
                <div class="flex justify-between bg-gray-300 px-2 py-1 items-center rounded-t-md">
                    <div class="text-lg font-semibold">Brands List</div>
                    <div class="flex">
                        ...
                        <Link v-if="add?.action_id || checkPermission" href="/brands/create" class="bg-green-600 p-2 rounded-r-md text-lg px-3 flex text-black text-white">
                            <Icon icon="fluent:add-12-filled" class="mr-2" width="1.5rem" height="1.5erm"/> Add Brands </Link>
                    </div>
                </div>
                ...
                <div class="px-3 h-[calc(100vh-13rem)] overflow-auto pb-3">
                    <div class="w-full border-t mt-4">
                        ...
                        <table class="w-full">
                            ...
                            <tbody>
                                <tr v-for="(brand, index) in Object.values(brands?.data)" class="odd:bg-white even:bg-gray-200 text-gray-800">
                                    <td class="text-center">{{ index+1 }}</td>
                                    <td><img v-if="brand?.icon" :src="`/brand/${brand?.icon}`" width="40px" height="auto"/></td>
                                    <td>{{ brand?.name }}</td>
                                    <td class="p-2">{{ brand?.description }}</td>
                                    <td class="p-2">{{ (brand?.status) ? 'Active' : 'Inactive' }}</td>
                                    <td class="flex gap-1 place-content-center">
                                        <Link v-if="edit?.action_id || checkPermission" :href="`/brands/${brand?.id}/edit`">
                    ...
                    <form @submit.prevent="submit">
                        <div class="grid grid-col-1 mt-4">
                            ...
                            <div class="card w-full justify-center mt-2">
                                <label for="dd-city" class="text-md w-full content-center font-semibold">Brand Name</label>
