...
const submit = () => {
    ...
    router.get('/order/paginate/filters', filters, {
        preserveState: true,
        replace: true,
                ...
                <div class="border rounded-b-md px-2 py-1 min-h-[calc(100vh-13rem)]">
                    <table class="table-fixed w-full">
                       <tr class="bg-gray-400">
                            ...
                            <tr v-for="(order, index) in Object.values(orders?.data)" class="bg-gray-100 even:bg-white cursor-pointer hover:bg-gray-300" :key="index" :class="colorClassMap[order.status.id]">
                                <td class="text-center">{{ ((orders?.current_page - 1) * orders?.per_page) + (index + 1) }}</td>
                                <td class="text-center">
                                    <Link :href="`/orders/${order.order_no}/view`">
                                        {{ dateFunction(order?.created_at) }}
                                    </Link>
                                </td>
                                ...
                                <td class="text-center p-1">
                                    <div class="flex gap-2">
                                        <div class="bg-sky-600 p-2 rounded-sm">
                                            <Link :href="`/orders/${order.order_no}/print`"><Icon icon="subway:print" class="text-white" width="1rem" height="1erm"/></Link>
                                        </div>
                                         <div class="bg-green-600 p-2 rounded-sm">
                                            <Link :href="`/orders/${order.order_no}/view`"><Icon icon="mdi:eye" class="text-white" width="1rem" height="1erm"/></Link>
                ...
                <form @submit.prevent="submit">
                    <div class="flex flex-col mt-4">
                        ...
                        <div class="flex flex-col col-span-2 w-full pt-1">
                            ...
                            <AutoComplete showClear v-model="selectedBranch" dropdown :suggestions="branches" optionLabel="name" size="small" @complete="branchSearch" inputClass="w-full text-sm border-gray-400"/>
                        </div>


                        <div class="flex flex-col col-span-2 w-full pt-1">
                            <label for="multiple-ac-1" class="block text-sm font-semibold">Brand</label>
                            <AutoComplete showClear v-model="selectedBrand" dropdown :suggestions="brands" optionLabel="name" size="small" inputClass="w-full text-sm border-gray-400" @complete="brandSearch"/>
                        </div>

                        <div class="flex flex-col col-span-2 w-full pt-1">
                            <label for="categories" class="text-sm w-full font-semibold">Category</label>
                            <AutoComplete v-model="selectedCategory" inputId="multiple-ac-1" optionLabel="name" size="small" inputClass="w-full text-sm" showClear dropdown :suggestions="categories" @complete="categorySearch"
                            />
                        </div>
                        ...
                        <div class="flex flex-col col-span-2 w-full pt-1">
                            ...
                            <AutoComplete v-model="selectedSize" optionLabel="size" size="small" inputClass="w-full text-sm" showClear dropdown :suggestions="sizes" @complete="sizeSearch" />
                        </div>

                        <div class="flex flex-col col-span-2 w-full pt-1">
                            <label for="multiple-ac-1" class="block text-sm font-semibold">Color</label>
                            <AutoComplete v-model="selectedColor" optionLabel="color" size="small" inputClass="w-full text-sm" showClear dropdown :suggestions="colors" @complete="colorSearch" />
                        </div>
