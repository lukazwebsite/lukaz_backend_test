...
const pageNumber = ref(props.subscribes?.current_page);
...
const submit = () => {
    ...
    router.post('/subscribe/paginate/filters', formData, {
        headers: {
...
const goToPage = () => {
    ...
    router.get('/subscribe?page=' + pageNumber.value, {}, {
        headers: {
            Accept: 'application/json',
            'X-Requested-With': 'XMLHttpRequest',
        },
        ...
        <div class="w-full mx-auto p-2 flex flex-row place-content-center">
            <div className="w-5/6 mt-4 shadow-lg bg-white rounded-md pb-1">
                <div class="flex justify-between bg-gray-300 px-2 py-1 items-center rounded-t-md">
                    <div class="text-lg font-semibold">Subscribe List</div>
                    <div class="flex">
                ...
                <div class="px-3 h-[calc(100vh-13rem)] overflow-auto pb-3">
                    <div class="w-full border-t mt-4">
                        <table class="w-full">
                            ...
                            <tbody>
                                <tr v-for="(subscribe, index) in Object.values(subscribes?.data)"
                                    class="odd:bg-white even:bg-gray-200 text-gray-800">
                                    <td class="text-center">{{ ((subscribes?.current_page - 1) * subscribes?.per_page) +
                                        (index + 1) }}</td>
                                    <td>{{ subscribe?.email }}</td>
                                    <td class="p-2">{{ (subscribe?.status) ? 'Active' : 'Inactive' }}</td>
                                    <td class="flex gap-1 place-content-center">
                                        <Link v-if="edit?.action_id" :href="`/subscribe/${subscribe?.id}/edit`">
                ...
                <Drawer v-model:visible="visibleRight" header="Subscribe Filter" position="right">
                    <form @submit.prevent="submit">
                        <div class="grid grid-col-1 mt-4">

                            <div class="card w-full justify-center mt-2">
                                <label for="dd-city" class="text-md w-full content-center font-semibold">Email</label>
