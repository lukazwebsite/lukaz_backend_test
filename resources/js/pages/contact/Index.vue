...
const pageNumber = ref(props.contacts?.current_page);
...
const submit = () => {
    ...
    router.post('/contact/paginate/filters', formData, {
        headers: {
...
const goToPage = () => {
    ...
    router.get('/contact?page=' + pageNumber.value, {}, {
        headers: {
            Accept: 'application/json',
            'X-Requested-With': 'XMLHttpRequest',
        },
        ...
        <div class="w-full mx-auto p-2 flex flex-row place-content-center">
            <div className="w-5/6 mt-4 shadow-lg bg-white rounded-md pb-1">
                <div class="flex justify-between bg-gray-300 px-2 py-1 items-center rounded-t-md">
                    <div class="text-lg font-semibold">Contact List</div>
                    <div class="flex">
                ...
                <div class="px-3 h-[calc(100vh-13rem)] overflow-auto pb-3">
                    <div class="w-full border-t mt-4">
                        <table class="w-full">
                            ...
                            <tbody>
                                <tr v-for="(contact, index) in Object.values(contacts?.data)"
                                    class="odd:bg-white even:bg-gray-200 text-gray-800">
                                    <td class="text-center">{{ ((contacts?.current_page - 1) * contacts?.per_page) +
                                        (index + 1) }}</td>
                                    <td>{{ contact?.name }}</td>
                                    <td>{{ contact?.email }}</td>
                                    <td>{{ contact?.phone }}</td>
                                    <td>{{ contact?.message }}</td>
                                    <td class="p-2">{{ (contact?.status) ? 'Active' : 'Inactive' }}</td>
                                    <td class="flex gap-1 place-content-center">
                                        <Link v-if="edit?.action_id" :href="`/contact/${contact?.id}/edit`">
                ...
                <Drawer v-model:visible="visibleRight" header="Contract Filter" position="right">
