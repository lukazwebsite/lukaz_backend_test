...
const pageNumber = ref(props.notices?.current_page);
...
const submit = () => {
    ...
    router.post('/notices/paginate/filters', formData, {
        headers: {
...
const goToPage = () => {
    ...
    router.get('/notices?page=' + pageNumber.value, {}, {
        headers: {
            Accept: 'application/json',
            'X-Requested-With': 'XMLHttpRequest',
        },
        ...
        <div class="w-full mx-auto p-2 flex flex-row place-content-center">
            <div className="w-5/6 mt-4 shadow-lg bg-white rounded-md pb-1">
                <div class="flex justify-between bg-gray-300 px-2 py-1 items-center rounded-t-md">
                    <div class="text-lg font-semibold">Notices List</div>
                    <div class="flex">
                        ...
                        <Link v-if="add?.action_id || checkPermission" href="/notices/create"
                            class="bg-green-600 p-2 rounded-r-md text-lg px-3 flex text-black text-white">
                        <Icon icon="fluent:add-12-filled" class="mr-2" width="1.5rem" height="1.5erm" /> Add Notices
                        </Link>
                    </div>
                </div>
                ...
                <div class="px-3 h-[calc(100vh-13rem)] overflow-auto pb-3">
                    <div class="w-full border-t mt-4">
                        <table class="w-full">
                            ...
                            <tbody>
                                <tr v-for="(notice, index) in Object.values(notices?.data)"
                                    class="odd:bg-white even:bg-gray-200 text-gray-800">
                                    <td class="text-center">{{ index + 1 }}</td>
                                    <td>{{ notice?.title }}</td>
                                    <td class="p-2">{{ notice?.description }}</td>
                                    <td class="p-2">{{ notice?.href }}</td>
                                    <td class="p-2">{{ notice?.button_text }}</td>
                                    <td class="flex gap-1 place-content-center">
                                        <Link v-if="edit?.action_id || checkPermission" :href="`/notices/${notice?.id}/edit`">
                ...
                <Drawer v-model:visible="visibleRight" header="Notice Filter" position="right">
                    <form @submit.prevent="submit">
                        <div class="grid grid-col-1 mt-4">
                            ...
                            <div class="card w-full justify-center mt-2">
                                <label for="dd-city"
                                    class="text-md w-full content-center font-semibold">Description</label>
                                <div class="card flex justify-center">
                            ...
                            <div class="card w-full justify-center mt-2">
                                <label for="dd-city" class="text-md w-full content-center font-semibold">Href</label>
                                <div class="card flex justify-center">
                                    <input v-model="href"
                                        class="w-full text-sm border py-1 px-2 rounded-md outline-none focus:border-green-200" />
                                </div>
                            ...
                            <div class="card w-full justify-center mt-2">
                                <label for="dd-city" class="text-md w-full content-center font-semibold">Button
                                    Text</label>
                                <div class="card flex justify-center">
