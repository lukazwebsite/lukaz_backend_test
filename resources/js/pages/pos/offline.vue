...
onMounted(() => {
    ...
    console.log('Menu Access →', menuAccess.value)
    console.log('Menu Access →', add)
});
...
const submit = () => {
    ...
    router.get('/offline/paginate/filters', filters, {
        preserveState: true,
        replace: true,
...
const goToPage = () => {
    ...
    router.get('/offline?page=' + pageNumber.value, {}, {
        headers: {
            ...
            <div class="w-5/6 mt-4 shadow-lg bg-white rounded-md">
                ...
                <div class="border rounded-b-md px-2 py-1 min-h-[calc(100vh-13rem)]">
                    <table class="table-fixed w-full">
                        ...
                        <tbody>
                            <tr v-for="(order, index) in Object.values(orders?.data)" class="bg-gray-100 even:bg-white cursor-pointer hover:bg-gray-300" :class="colorClassMap[order.status.id]" :key="order?.id">
                                <td class="text-center">{{ ((orders?.current_page - 1) * orders?.per_page) + (index + 1) }}</td>
                                <td>
                                    <Link :href="`/orders/${order.order_no}/view`" class="hover:text-blue-700 text-blue-400">{{ dateFunction(order?.created_at) }}</Link>
                                ...
                                <td class="text-left">{{ order?.branch?.name }}</td>
                                <td class="text-center">{{ order?.status?.name }}</td>
                                <td class="text-center p-1">
                                    <div class="flex gap-2">
                                        <div class="bg-sky-600 rounded-sm">
                                            <Link :href="`/orders/${order?.order_no}/print`" title="Print" class="block p-2"><Icon icon="subway:print" class="text-white" width="1rem" height="1erm"/></Link>
                                        </div>
                                        <div class="bg-green-600 rounded-sm">
                                            <Link :href="`/orders/${order?.order_no}/view`" class="block p-2"><Icon icon="mdi:eye" class="text-white" width="1rem" height="1erm"/></Link>
                                        </div>
                                        <div class="bg-gray-600 rounded-sm">
                                            <a target="_blank" rel="noopener noreferrer" class="block p-2" title="Pos Print" :href="`/offline/${order?.order_no}/print`"><Icon icon="mdi:printer-pos" class="text-white " width="1rem" height="1erm"/></a>
                                        </div>
                                    </div>
                    ...
                    <div class="flex flex-col mt-4">
                        ...
                        <div class="card w-full justify-center mt-2">
                            ...
                            <div class="card flex justify-center">
                                <input v-model="name" class="w-full text-sm border border-gray-400 py-1 px-2 rounded-md outline-none focus:border-green-200" placeholder="Price,Name,Order No"/>
                            </div>
                        ...
                        <div class="card w-full justify-center">
                            ...
                            <select v-model="status_id" class="w-full text-md border border-gray-400 py-1 px-2 outline-none focus:border-green-200 rounded-md">
                                <option value="">Select Status</option>
                                <option v-for="sta in status" :value=sta.id class="text-red-500" :key="sta.id">{{ sta.name }}</option>
                            </select>
                        </div>
