<script setup lang="ts">
    ...
    const paidAmount = computed(() => (props.order?.transactions ?? [])
        .filter(txn => txn.payment_method !== 'discount')
        .reduce((acc, txn) => acc + Number(txn.credit || 0), 0));

    const dueAmount = computed(() => Number(props.order?.grand_total || 0) - paidAmount.value);

    const lineTotal = (item) => (item?.quantity * item?.regular_price) - Number(item?.discount_amount || 0);
                    ...
                    <template v-for="item in order?.items" :key="item.id">
                        ...
                        <tr>
                            ...
                            <td class="px-1 font-semibold  flex gap-2 text-[8px]">
                                <del v-if="item?.deleted_at != null" > ৳  {{ lineTotal(item) }}</del>
                                <span v-else>৳ {{ lineTotal(item) }}</span>
                                <span v-if="item?.discount_amount > 0" class="text-[7px]">(-৳ {{ item?.discount_amount }})</span>
                            </td>
                        </tr>
                    </template>
                </tbody>
            ...
            <div class="flex justify-end">
                <div class="w-full max-w-xs mt-2">
                    <div class="px-2 text-xs flex border-b border-gray-200">
                        ...
                        <span class="w-full text-right">৳ {{ order?.total }} </span>
                    </div>
                    ...
                    <div v-if="order?.additional_discount > 0" class="px-2 text-xs font-semibold flex justify-end border-b border-gray-200">
                        <span class="w-5/7 text-right">Additional Discount : </span>
                        <span class="w-full text-right">৳ {{ order?.additional_discount }} </span>
                    </div>
                    <div class="flex justify-between px-2 border-gray-200 pt-2">
                        <span class="text-lg font-semibold text-gray-800">Total:</span>
                        <span class="text-lg font-semibold text-gray-800"><Currency :amount="order?.grand_total"/></span>
                    </div>
                    <div class="flex justify-between px-2 border-gray-200 pt-1">
                    ...
                    <div v-if="dueAmount > 0" class="flex px-2 border-gray-200 pt-2 text-center justify-center items-center">
                        <span class="text-lg font-semibold text-gray-800">Due:</span>
                        <span class="text-lg font-semibold text-gray-800 w-full"><Currency :amount="dueAmount"/></span>
                    </div>
                    <div v-else-if="dueAmount < 0" class="flex px-2 border-gray-200 pt-2 text-center justify-center items-center">
                        <span class="text-lg font-semibold text-gray-800">Return:</span>
                        <span class="text-lg font-semibold text-gray-800 w-full"><Currency :amount="Math.abs(dueAmount)"/></span>
                    </div>
