...
const teamMemberId = ref(props.teamMember?.id);
const name = ref(props.teamMember?.name);
const designation = ref(props.teamMember?.designation);
const whatsapp = ref(props.teamMember?.whatsapp);
const sortOrder = ref(props.teamMember?.sort_order);
const image = ref('');
const imagePreview = ref('');
const status = ref(props.teamMember?.status);
const errors = ref({});
...
const submit = () => {
    ...
    router.post('/team_member/' + teamMemberId?.value, formData, {
        headers: {
        ...
        forceFormData: true,
        onSuccess: () => {
            toast.add({ severity: 'success', summary: 'Success Message', detail: 'Team Member update successfully!', life: 3000 });
            errors.value = {};
            loading.value = false;
        },
        onError: (e) => {
        ...
        <div class="w-full mx-auto p-2 flex flex-row place-content-center">
            <div className="w-5/6 mt-4 shadow-lg bg-white rounded-md pb-1">
                ...
                <div class="flex justify-between bg-gray-300 px-2 items-center rounded-t-md p-1 ">
                    <div class="text-lg font-semibold">Edit Team Member</div>
                    <div class="flex">
                ...
                <form @submit.prevent="submit" class="px-3 min-h-[calc(100vh-12rem)] justify-items-center mt-2">
                    ...
                    <div class="w-3/4 rounded-md  p-2 px-4 grid grid-cols-2 gap-2">
                        ...
                        <div class="w-full mb-1">
                            ...
                            <img v-if="imagePreview" :src="imagePreview" class="mt-2 h-24 w-24 object-cover rounded-md"
                                alt="Preview" />
                            <img v-else-if="teamMember?.image" :src="`/team/${teamMember?.image}`"
                                class="mt-2 h-24 w-24 object-cover rounded-md" alt="Current" />
                        </div>
                    ...
                    <div class="mb-1 w-3/4 flex justify-end mt-2">
                        <button
                            class="justify-items-end cursor-pointer bg-sky-500 hover:bg-sky-600 text-md border py-1 px-2 rounded-md font-semibold text-white">Update
                            Team Member</button>
                    </div>
