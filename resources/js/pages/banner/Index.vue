...
const pageNumber = ref(props.banners?.current_page);
...
const submit = () => {
    ...
    router.post('/banner/paginate/filters', formData, {
        headers: {
...
const goToPage = () => {
    ...
    router.get('/banner?page=' + pageNumber.value, {}, {
        headers: {
            Accept: 'application/json',
            'X-Requested-With': 'XMLHttpRequest',
        },
        ...
        <div class="w-full mx-auto p-2 flex flex-row place-content-center">
            <div className="w-5/6 mt-4 shadow-lg bg-white rounded-md pb-1">
                <div class="flex justify-between bg-gray-300 px-2 py-1 items-center rounded-t-md">
                    <div class="text-lg font-semibold">Banner List</div>
                    <div class="flex">
                        ...
                        <Link v-if="add?.action_id || checkPermission " href="/banner/create"
                            class="bg-green-600 p-2 rounded-r-md text-lg px-3 flex text-black text-white">
                        <Icon icon="fluent:add-12-filled" class="mr-2" width="1.5rem" height="1.5erm" /> Add Banner
                        </Link>
                    </div>
                </div>
                ...
                <div class="px-3 h-[calc(100vh-13rem)] overflow-auto pb-3">
                    <div class="w-full border-t mt-4">
                        <table class="w-full">
                            ...
                            <tbody>
                                <tr v-for="(banner, index) in Object.values(banners?.data)"
                                    class="odd:bg-white even:bg-gray-200 text-gray-800">
                                    <td class="text-center">{{ ((banners?.current_page - 1) * banners?.per_page) +
                                        (index + 1) }}</td>
                                    <td><img v-if="banner?.image" :src="`/banners/${banner?.image}`" width="40px"
                                            height="auto" /></td>
                                    <td>{{ banner?.title }}</td>
                                    <td>{{ banner?.description }}</td>
                                    <td>{{ banner?.sequence }}</td>
                                    <td class="p-2">{{ (banner?.status) ? 'Active' : 'Inactive' }}</td>
                                    <td class="flex gap-1 place-content-center">
                                        <Link v-if="edit?.action_id || checkPermission" :href="`/banner/${banner?.id}/edit`">
                ...
                <Drawer v-model:visible="visibleRight" header="Banner Filter" position="right">
