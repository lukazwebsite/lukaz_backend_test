...
const staffId = ref(props.customer?.id);
const name = ref(props.customer?.name);
const password = ref('');
const confirm = ref('');
const email = ref(props.customer?.email);
const mobile = ref(props.customer?.mobile);
const roleId = ref(props.customer?.role_id);
const branchId = ref(props.customer?.branch_id);
const status = ref(props.customer?.status);
const joining = ref(props.customer?.joining);
const description = ref(props.customer?.description);
const errors = ref({});
...
const submit = () => {
    ...
    router.post('/customer/' + staffId.value, formData, {
        headers: {
                    ...
                    <div class="w-3/4 rounded-md  p-2 px-4 grid grid-cols-2 gap-2">
                        ...
                        <div class="grid mb-1 col-span-2">
                            ...
                            <textarea v-model="description"
                                class="w-full text-md border py-1 px-2 outline-none focus:border-green-200 rounded-md"
                                style="height: 120px">{{ description }}</textarea>
                        </div>
