...
const pageNumber = ref(props.customers?.current_page);
...
const submit = () => {
    ...
    router.post('/customer/paginate/filters', formData, {
        headers: {
...
const goToPage = () => {
    ...
    router.get('/customer?page=' + pageNumber.value, {}, {
        headers: {
            Accept: 'application/json',
            'X-Requested-With': 'XMLHttpRequest',
        },
            ...
            <div className="w-5/6 mt-4 shadow-lg bg-white rounded-md pb-1">
                <div class="flex justify-between bg-gray-300 px-2 py-1 items-center rounded-t-md">
                    ...
                    <div class="flex">
                        ...
                        <Link v-if="add?.action_id || checkPermission" href="/customer/create"
                            class="bg-green-600 p-2 rounded-r-md text-lg px-3 flex text-black text-white">
                        <Icon icon="fluent:add-12-filled" class="mr-2" width="1.5rem" height="1.5erm" /> Add Customer
                        </Link>
                    </div>
                </div>
                ...
                <div class="px-3 h-[calc(100vh-13rem)] overflow-auto pb-3">
                    <div class="w-full border-t mt-4">
                        <table class="w-full">
                            ...
                            <tbody>
                                <tr v-for="(customer, index) in Object.values(customers?.data)"
                                    class="odd:bg-white even:bg-gray-200 text-gray-800">
                                    <td class="text-center">{{ ((customers?.current_page - 1) * customers?.per_page) +
                                        (index + 1) }}</td>
                                    ...
                                    <td class="p-2">{{ customer?.joining }}</td>
                                    <td class="p-2">{{ (customer?.status) ? 'Active' : 'Inactive' }}</td>
                                    <td class="flex gap-1 place-content-center">
                                        <Link v-if="edit?.action_id || checkPermission" :href="`/customer/${customer?.id}/edit`">
                ...
                <!-- main content goes here -->
                <Drawer v-model:visible="visibleRight" header="Customer Filter" position="right">
