<script setup lang="ts">
    ...
    const sendToSteadfast = () => {
        ...
        router.post(`/steadfast/order/${props?.orders?.order_no}/send`, {}, {
            preserveScroll: true,
            onFinish: () => {
                sending.value = false;
            },
    ...
    const orderUpdate = () => {
        ...
        router.post('/orders/'+orderNo.value+'/extra', formData, {
            headers: {
    ...
    const itemUpdate = (additional) => {
        ...
        router.get(`/orders/${additional}/items_update/extra`, {
            headers: {
                Accept: 'application/json',
                'X-Requested-With': 'XMLHttpRequest',
            },
                    ...
                    <div class="w-full mx-auto">
                        <div class="bg-white overflow-hidden">
                                    ...
                                    <table class="w-full">
                                        ...
                                        <tbody class="divide-y divide-gray-200">
                                            <tr v-for="order in orders?.items">
                                                ...
                                                <td class="px-4 py-4 text-gray-900 text-sm">
                                                    <del v-if="order?.deleted_at != null">{{ (order?.quantity * order?.current_price) }}</del>
                                                    <span v-else>{{ (order?.quantity * order?.current_price) }}</span>
                                                </td>
                            ...
                            <div class="py-4 border-t flex justify-between px-4">
                                <Link href="/orders/extra" class="bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded-lg flex items-center">
