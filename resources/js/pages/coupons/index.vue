<script setup lang="ts">
    ...
    const pageNumber = ref(props.coupons?.current_page);
...
const submit = () => {
        ...
        router.post('/coupon/paginate/filters', formData, {
            headers: {
    ...
    const goToPage = () => {
        ...
        router.get('/coupon?page=' + pageNumber.value, {}, {
            headers: {
                Accept: 'application/json',
                'X-Requested-With': 'XMLHttpRequest',
            },
        ...
        <div class="w-full mx-auto p-2 flex flex-row place-content-center">
            <div className="w-5/6 mt-4 shadow-lg bg-white rounded-md pb-1">
                <div class="flex justify-between bg-gray-300 px-2 py-1 items-center rounded-t-md">
                    <div class="text-lg font-semibold">Coupon List</div>
                    <div class="flex">
                        ...
                        <Link v-if="add?.action_id || checkPermission" href="/coupon/create" class="bg-green-600 p-2 rounded-r-md text-lg px-3 flex text-black text-white">
                            <Icon icon="fluent:add-12-filled" class="mr-2" width="1.5rem" height="1.5erm"/> Add Coupon </Link>
                    </div>
                </div>
                ...
                <div class="px-3 h-[calc(100vh-13rem)] overflow-auto pb-3">
                    <div class="w-full border-t mt-4">
                        <table class="w-full">
                            ...
                            <tbody>
                                <tr v-for="(coupon, index) in Object.values(coupons?.data)" class="odd:bg-white even:bg-gray-200 text-gray-800">
                                    <td class="text-center">{{ ((coupons?.current_page - 1) * coupons?.per_page) + (index + 1) }}</td>
                                    <td><img v-if="coupon?.icon" :src="`/coupons/${coupon?.icon}`" width="40px" height="auto"/></td>
                                    <td>{{ coupon?.name }}</td>
                                    <td>{{ coupon?.slug }}</td>
                                    <td>{{ coupon?.discount }}{{ coupon?.discount_type ? 'tk': '%' }}</td>
                                    <td>{{ coupon?.limit }}</td>
                                    <td><Currency :amount="coupon?.max_discount"/></td>
                                    <td>{{ dateFunction(coupon?.start_date) }}</td>
                                    <td class="p-2">{{ dateFunction(coupon?.end_date) }}</td>
                                    <td class="p-2">{{ (coupon?.status) ? 'Active' : 'Inactive' }}</td>
                                    <td class="flex gap-1 place-content-center">
                                        <Link v-if="edit?.action_id || checkPermission" :href="`/coupon/${coupon?.id}/edit`">
                    ...
                    <form @submit.prevent="submit">
                        <div class="grid grid-col-1 mt-4">
                            ...
                            <div class="card w-full justify-center mt-2">
                                <label for="dd-city" class="text-md w-full content-center font-semibold">Coupon Name</label>
                            ...
                            <div class="card w-full justify-center">
                                <label for="dd-city" class="text-md w-full content-center font-semibold">Discount Type</label>
                                <select v-model="discountType" class="w-full text-md border py-1 px-2 outline-none focus:border-green-200 rounded-md">
