...
const pageNumber = ref(props.shopbys?.current_page);

const breadcrumbs = ref([
    { title: 'Dashboard', href: '/' },
    { title: 'Shop By', href: '/shop_by' }
]);
...
const submit = () => {
    ...
    router.post('/shop_by/paginate/filters', formData, {
        headers: {
...
const goToPage = () => {
    ...
    router.get('/shop_by?page=' + pageNumber.value, {}, {
        headers: {
            Accept: 'application/json',
            'X-Requested-With': 'XMLHttpRequest',
        },
        ...
        <div class="w-full mx-auto p-2 flex flex-row place-content-center">
            <div className="w-5/6 mt-4 shadow-lg bg-white rounded-md pb-1">
                <div class="flex justify-between bg-gray-300 px-2 py-1 items-center rounded-t-md">
                    <div class="text-lg font-semibold">Shop By List</div>
                    <div class="flex">
                        ...
                        <Link v-if="add?.action_id || checkPermission" href="/shop_by/create"
                            class="bg-green-600 p-2 rounded-r-md text-lg px-3 flex text-black text-white">
                        <Icon icon="fluent:add-12-filled" class="mr-2" width="1.5rem" height="1.5erm" /> Add Shop By
                        </Link>
                    </div>
                </div>
                ...
                <div class="px-3 h-[calc(100vh-13rem)] overflow-auto pb-3">
                    <div class="w-full border-t mt-4">
                        <table class="w-full">
                            ...
                            <tbody>
                                <tr v-for="(shopby, index) in Object.values(shopbys?.data)" class="odd:bg-white even:bg-gray-200 text-gray-800" :key="index">
                                    <td class="text-center">{{ ((shopbys?.current_page - 1) * shopbys?.per_page) +
                                        (index + 1) }}</td>
                                    <td><img v-if="shopby?.thumbnail" :src="`/shopby/${shopby?.thumbnail}`" width="40px" height="auto" /></td>
                                    <td>{{ shopby?.name }}</td>
                                    <td>

                                        <span v-for="(tag, index) in shopby?.categories">{{ tag?.name }}<span v-if="index < shopby?.categories?.length - 1">, </span></span>



                                    </td>
                                    <td class="p-2">{{ (shopby?.status) ? 'Active' : 'Inactive' }}</td>
                                    <td class="flex gap-1 place-content-center">
                                        <Link v-if="edit?.action_id || checkPermission" :href="`/shop_by/${shopby?.id}/edit`">
                ...
                <div class="flex justify-between w-full px-3 py-2 border-t">
                    ...
                    <div>
                        ...
                        <Pagination :links="shopbys?.links" />
                    </div>
                ...
                <Drawer v-model:visible="visibleRight" header="Shop By Filter" position="right">
