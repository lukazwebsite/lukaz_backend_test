...
class ExtraOrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $orders = $query->paginate(20)->onEachSide(1)->appends($append)->withPath('/order/paginate/filters/extra');
        ...
        return Inertia::render('extra_order/index', [
    ...
    public function show(Request $request,string $id)
    {
        ...
        $query = Order::withCount('items')->with(['status', 'user', 'items.product', 'transactions'])->where('status', $id)->orderBy('id', 'desc');
        ...
        $orders = $query->paginate(20)->onEachSide(1)->appends($append)->withPath('/order/paginate/filters/'.$id.'/show/extra');
        $status = Status::get(['id', 'name']);
        $categories = Category::where('status', 1)->get(['id', 'name']);
        $brands = Brand::where('status', 1)->get(['id', 'name']);
        ...
        return Inertia::render('extra_order/filter', [
            'filter_status' => $id,
            'menuAccess' => $menuAccess,
            'orders' => $orders,
            'orders' => $orders,
    ...
    public function preorder(Request $request, $id){
        ...
        $branches = Branch::get(['id', 'name']);
        $query = Order::withCount('items')->with(['status', 'user', 'items.product', 'transactions'])->orderBy('id', 'desc');
        ...
        if ($request->has('brand') && $request->brand !== null) {
            ...
            $query->whereExists(function($sub) use($request){
                ...
                if (!empty($request->brand)) {
                    $sub->where('brand_id', $request->brand);
                }
            });
        ...
        if ($request->has('category') && $request->category !== null) {
            $query->whereExists(function($sub) use($request){
                ...
                if (!empty($request->category)) {
                    $sub->whereJsonContains('p.category_ids', $request->category);
                }
            });
        ...
        $orders = $query->paginate(20)->onEachSide(1)->appends($append)->withPath('/orders/'.$id.'/show/preorder/extra');
        $status = Status::get(['id', 'name']);
        $categories = Category::where('status', 1)->get(['id', 'name']);
        $brands = Brand::where('status', 1)->get(['id', 'name']);
        ...
        return Inertia::render('extra_order/preorder', [
            'filter_status' => $id,
            'menuAccess' => $menuAccess,
            'orders' => $orders,
            'branches' => $branches,
    ...
    public function view(Request $request, $order)
    {
        ...
        return Inertia::render('extra_order/view', [
            'menuAccess' => $menuAccess,
            'orders' => $order,
            'status' => $status,
    ...
    public function print(Request $request, $order)
    {
        ...
        return Inertia::render('extra_order/print', [
            'menuAccess' => $menuAccess,
            'orders' => $order,
            'status' => $status,
        ]);
    ...
    public function update(Request $request, string $orderNo)
    {
       $checkPermission = Permission::access($request, $this->menuId, 3);
        ...
        }elseif(($request->status_id == 6 && $checkStatus->status != 6) || ($request->status_id == 7 && $checkStatus->status != 7) ){
