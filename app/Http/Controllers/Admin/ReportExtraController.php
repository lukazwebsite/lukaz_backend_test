...
class ReportExtraController extends Controller
{
    private $menuId = 38;
    /**
    ...
    public function index(Request $request, Excel $excel)
    {
        ...
        if($request->excel == "download"){
            ...
            return $excel->download(new ProductSaleExport($stocks), 'Sales_Reporst_'.date("j_m_Y_h_i_s", strtotime(Carbon::now())).'.xlsx');
        }



        $stocks = $query->orderBy('orderitems_sum_quantity', 'desc')->paginate(15)->withQueryString();

        return Inertia::render('reportextra/index', [
    ...
    public function sale(Request $request, Excel $excel)
    {
        ...
        $query = Order::with(['items', 'items.additional.product', 'items.additional.product.categories'])
            ...
            $sales = $query->paginate(5)->onEachSide(1)->appends($append);

        return Inertia::render('reportextra/sale', [
            'sales' => $sales,
            'sizes' => $sizes,
            'colors' => $colors,
            'branches' => $branches,
    ...
    public function stocks(Request $request)
    {
        ...
        return Inertia::render('reportextra/stocks', [
            'append' => $append,
            'sizes' => $sizes,
            'colors' => $colors,
            'branches' => $branches,
