...
class VendorController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $vendors = $query->latest()->paginate(10)->appends($append)->withPath('/vendors/paginate/filters');
    ...
    public function store(Request $request)
    {
        ...
        if($request->hasFile('banner')){
            if (!File::exists(public_path('/vendor/banner'))) {
                File::makeDirectory(public_path('/vendor/banner'), 0755, true);
            }
            ...
            $image->save(public_path('/vendor/banner').'/'.$name);

        }

        if($request->hasFile('thumbnail')){
            if (!File::exists(public_path('/vendor/thumbnail'))) {
                File::makeDirectory(public_path('/vendor/thumbnail'), 0755, true);
            }
            ...
            $image->save(public_path('/vendor/thumbnail').'/'.$name);

        }

        if($request->hasFile('icon')){
            if (!File::exists(public_path('/vendor/icon'))) {
                File::makeDirectory(public_path('/vendor/icon'), 0755, true);
            }
            ...
            $iconName =  'icon/'.$name;
            $data['icon'] = $iconName;
            $image->save(public_path('/vendor/icon').'/'.$name);
    ...
    public function update(Request $request, string $id)
    {
        ...
        return redirect()->route('vendor.index')->with('success', 'Brand updated successfully.');
    }
    ...
    public function destroy(Request $request, string $id)
    {
        ...
        return redirect()->route('vendor.index')->with('success', 'Brand deleted successfully.');
    }
}
